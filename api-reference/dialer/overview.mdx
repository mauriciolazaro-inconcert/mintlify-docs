...
Envía `POST /batches` con `source: "api"`, el nombre y el canal. Puedes incluir `duplicateStrategy` y `rules` para dejar configuradas la deduplicación, los intentos, la ventana horaria, las prioridades y el comportamiento ante cada resultado desde la creación.

El lote se crea sin contactos. Añádelos después mediante `POST /batches/{batchId}/contacts`.
